...
if archivo_cargado:
    ...
    prompt_react_es = PromptTemplate(
        ...
        template="""
            ...
            Final Answer: Escribe aquí la respuesta final clara y directa en castellano. No repitas el proceso ni simules nuevas preguntas.
    ...
    if st.button("📄 Reporte de Informaciones Generales", key="boton_reporte_general"):
        with st.spinner("Generando Reporte 🦜"):
            respuesta = orquestador.invoke({"input": "Quiero un reporte con informacion de los datos"})
   ...
   # PERGUNTA SOBRE LOS DATOS
    ...
    pregunta_sobre_datos = st.text_input("Realiza una pregunta sobre los datos (ej: 'Cuál es el promedio de -columna-?')")
    ...
    pregunta_grafico = st.text_input("Qué deseas visualizar? (ej: 'Genera un gráfico del promedio de -columna-.')")
