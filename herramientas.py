...
def generar_grafico(pregunta: str, df:pd.DataFrame) -> str:
    ...
    agents y el tiempo de entrega', entre otros. Las Palabras-clave comunes que indican el uso de 
    ...
    # SE REDEFINEN LAS REGLAS DENTRO DE LA PLANTILLA PARA EVITAR EL RECORTE DE DATOS
    plantilla_respuesta = PromptTemplate( 
        template = """
        Eres un especialista en visualización de datos con Python, Matplotlib y Seaborn. Tu tarea es generar **únicamente el código Python** ejecutable para graficar con base en la 	solicitud del usuario, utilizando el DataFrame llamado `df`.
        ...
        ## Muestra de los datos (3 primeras filas de referencia estructural):
        {muestra}

        ## INSTRUCCIONES OBLIGATORIAS DE GENERACIÓN DE CÓDIGO:
        1. Usa las bibliotecas `matplotlib.pyplot` (como `plt`) y `seaborn` (como `sns`).
        2. Define el tema con `sns.set_theme()`.
        3. DEBES utilizar el 100% de las filas del DataFrame `df`. Está TERMINANTEMENTE PROHIBIDO usar `.head()`, `.iloc[:N]`, muestras aleatorias o recortes artificiales que reduzcan la 	   cantidad de datos en el gráfico final, a menos que el usuario lo pida explícitamente en su pregunta.
        4. Si la petición requiere una línea temporal o el eje X contiene fechas, convierte OBLIGATORIAMENTE la columna a formato datetime usando 'pd.to_datetime()' y ordena el DataFrame 	   cronológicamente antes de graficar para asegurar la continuidad de los puntos.
        5. Configura el tamaño del gráfico con `figsize=(10, 5)` para evitar que los datos se amontonen.
        ...
        8. Si hay muchas categorías en el eje X, puedes rotar las etiquetas con `plt.xticks(rotation=45)` para que sean legibles; si son pocas, mantenlas sin rotación con 	  	 	`plt.xticks(rotation=0)`.
        9. Elimina los bordes superior y derecho del gráfico con `sns.despine()`.
        10. Usa `plt.tight_layout()` al final para evitar que se corten las etiquetas de los ejes al renderizar.
        11. Finaliza el código con `plt.show()`. No incluyas bloques de Markdown (```python) ni comentarios ni explicaciones en prosa fuera del código. Retorna EXCLUSIVAMENTE las líneas de 	    código listas para ser ejecutadas por `exec()`.
